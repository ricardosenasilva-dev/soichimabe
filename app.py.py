...
def normalize_turma_df(df, serie_name=None):
  ...
  std = pd.DataFrame()
  # Raw assignment
  if ra_col and ra_col in df.columns:
    ra_raw = df[ra_col].astype(str)
  else:
    ra_raw = df.index.astype(str)
  # Clean RA: remove trailing .0 from floats and whitespace
  def _clean_ra_value(v):
    s = str(v).strip()
    if s.endswith('.0'):
      try:
        # convert float-like string to int representation
        s = str(int(float(s)))
      except Exception:
        s = s[:-2]
    # remove stray decimal if integer-like '123.00'
    if re.match(r'^\d+\.0+$', s):
      try:
        s = str(int(float(s)))
      except Exception:
  ...
  std['RA'] = ra_raw.apply(_clean_ra_value) if hasattr(ra_raw, 'apply') else _clean_ra_value(ra_raw)

  # Nome
  if nome_col and nome_col in df.columns:
    std['Nome'] = df[nome_col].astype(str).str.strip()
  else:
    # fallback: try best non-empty column for name
    if df.shape[1] > 0:
      # prefer first non-numeric column
      chosen = None
      for c in df.columns:
        try:
          # if entire column not numeric, use as name
      ...
      if not chosen:
        chosen = df.columns[0]
      std['Nome'] = df[chosen].astype(str).str.strip()
    else:
      std['Nome'] = ''
      ...
      if perfil_atual in ["Gestão", "Administrador"]:
              ...
              if df_turma_g.empty:
                ...
                with st.form(key=f"form_chamada_turma_gestao_{turma_chamada_gestao}"):
                  ...
                  for idx, row in df_turma_g.iterrows():
                                        ra_display_g = str(row.get('RA','')).strip()
                                        nome_display_g = str(row.get('Nome','')).strip()
                                        if ra_display_g.endswith('.0'):
                                          try:
                                            ra_display_g = str(int(float(ra_display_g)))
                                          except Exception:
                                            ra_display_g = ra_display_g[:-2]
                                        ra_key_g = re.sub(r"[^A-Za-z0-9]", "_", ra_display_g) if ra_display_g else str(idx)
                                        falta_marcada_g = st.checkbox(
                                            f"RA: {ra_display_g} — {nome_display_g}", key=f"faltoso_gestao_{turma_chamada_gestao}_{ra_key_g}",
                                        )
                                        if falta_marcada_g:
                                          lista_faltosos_g.append(ra_display_g)
                  ...
                  if btn_salvar_chamada_g:
                    ...
                    # Registrar histórico diário de presença para Gestão
                    ATT_DIR_G = os.path.join(BASE_DIR, "presencas_diarias", turma_chamada_gestao)
                    os.makedirs(ATT_DIR_G, exist_ok=True)
                    att_file_g = os.path.join(ATT_DIR_G, f"{data_chamada_gestao.isoformat()}.csv")
                    try:
                        df_att_g = pd.DataFrame([
                            {
                                "RA": str(r.get("RA", "")).strip(),
                                "Nome": str(r.get("Nome", "")).strip(),
                                "Status": ("Falta" if str(r.get("RA", "")).strip() in lista_faltosos_g else "Presença"),
                            }
                            for _, r in df_turma_g.iterrows()
                        ])
                        df_att_g.to_csv(att_file_g, sep=";", index=False, encoding="latin1")
                    except Exception as e:
                        st.warning(f"Aviso: não foi possível salvar histórico de presença (Gestão): {e}")
        ...
        with aba_relatorios:
          ...
          if df_alunos_audit.empty:
            ...
            st.markdown("---")
            st.markdown("### 📚 Histórico de Presenças (por Turma)")
            turma_hist = st.selectbox(
                "Selecione a Turma para ver histórico:",
                options=SERIES_TURMAS,
                key="gest_hist_turma",
            )
            hist_dir = os.path.join(BASE_DIR, "presencas_diarias", turma_hist)
            if os.path.exists(hist_dir):
                files = [f for f in os.listdir(hist_dir) if f.lower().endswith('.csv')]
                if files:
                    files_sorted = sorted(files, reverse=True)
                    sel_file = st.selectbox(
                        "Selecione a data (arquivo):",
                        options=files_sorted,
                        key="gest_hist_file",
                    )
                    if sel_file:
                        sel_path = os.path.join(hist_dir, sel_file)
                        try:
                            df_hist_g = robust_read_csv(sel_path)
                            st.dataframe(df_hist_g, use_container_width=True, hide_index=True)
                            csv_text_g = df_hist_g.to_csv(sep=';', index=False)
                            st.download_button(
                                "📥 Baixar CSV de Histórico",
                                csv_text_g.encode('latin1', 'replace'),
                                file_name=f"{turma_hist}_{sel_file}",
                                mime="text/csv",
                            )
                        except Exception as e:
                            st.warning(f"Não foi possível carregar histórico: {e}")
                else:
                    st.info("Nenhum histórico diário registrado para esta turma.")
            else:
                st.info("Nenhum histórico encontrado para esta turma.")
      ...
      # ==============================================================================
            ...
            if df_turma_aoe.empty:
              ...
              with st.form(key=f"form_chamada_turma_{turma_chamada_aoe}"):
                ...
                for idx, row in df_turma_aoe.iterrows():
                                    ra_display = str(row.get('RA','')).strip()
                                    nome_display = str(row.get('Nome','')).strip()
                                    # limpar RA para exibição e para chave do widget
                                    if ra_display.endswith('.0'):
                                      try:
                                        ra_display = str(int(float(ra_display)))
                                      except Exception:
                                        ra_display = ra_display[:-2]
                                    ra_key = re.sub(r"[^A-Za-z0-9]", "_", ra_display) if ra_display else str(idx)
                                    falta_marcada = st.checkbox(
                                        f"RA: {ra_display} — {nome_display}",
                                        key=f"faltoso_{turma_chamada_aoe}_{ra_key}",
                                    )
                                    if falta_marcada:
                                      lista_faltosos.append(ra_display)
                ...
                if btn_salvar_chamada:
                  ...
                  # Registrar histórico diário de presença
                  ATT_DIR = os.path.join(BASE_DIR, "presencas_diarias", turma_chamada_aoe)
                  os.makedirs(ATT_DIR, exist_ok=True)
                  att_file = os.path.join(ATT_DIR, f"{data_chamada_aoe.isoformat()}.csv")
                  try:
                      df_att = pd.DataFrame([
                          {
                              "RA": str(r.get("RA", "")).strip(),
                              "Nome": str(r.get("Nome", "")).strip(),
                              "Status": ("Falta" if str(r.get("RA", "")).strip() in lista_faltosos else "Presença"),
                          }
                          for _, r in df_turma_aoe.iterrows()
                      ])
                      df_att.to_csv(att_file, sep=";", index=False, encoding="latin1")
                  except Exception as e:
                      st.warning(f"Aviso: não foi possível salvar histórico de presença: {e}")
        ...
        with aba_aoe_relatorio:
          ...
          if df_alunos_audit_aoe.empty:
            ...
            st.markdown("---")
            st.markdown("### 📚 Histórico de Presenças (por Turma)")
            turma_hist_aoe = st.selectbox(
                "Selecione a Turma para ver histórico:",
                options=SERIES_TURMAS,
                key="aoe_hist_turma",
            )
            hist_dir_aoe = os.path.join(BASE_DIR, "presencas_diarias", turma_hist_aoe)
            if os.path.exists(hist_dir_aoe):
                files = [f for f in os.listdir(hist_dir_aoe) if f.lower().endswith('.csv')]
                if files:
                    files_sorted = sorted(files, reverse=True)
                    sel_file = st.selectbox(
                        "Selecione a data (arquivo):",
                    ...
                    if sel_file:
                        sel_path = os.path.join(hist_dir_aoe, sel_file)
                        try:
                            df_hist = robust_read_csv(sel_path)
                            st.dataframe(df_hist, use_container_width=True, hide_index=True)
                            csv_text = df_hist.to_csv(sep=';', index=False)
                            st.download_button(
                                "📥 Baixar CSV de Histórico",
                                csv_text.encode('latin1', 'replace'),
                                file_name=f"{turma_hist_aoe}_{sel_file}",
                                mime="text/csv",
                            )
                        except Exception as e:
                            st.warning(f"Não foi possível carregar histórico: {e}")
