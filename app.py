...
if os.path.exists(LOGO_PATH):
    ...
    st.sidebar.warning(f"⚠️ Logotipo '{LOGO_PATH}' não encontrado na pasta do projeto.")
...
if not os.path.exists(OCORRENCIAS_CSV):
    df_oc_init = pd.DataFrame(columns=["Data", "RA", "Nome", "Série", "Gravidade", "Descricao", "Professor", "MensagemGestaoAOE"])
    df_oc_init.to_csv(OCORRENCIAS_CSV, index=False)

if not os.path.exists(COMUNICADOS_CSV):
    df_com_init = pd.DataFrame(columns=["Data", "Remetente", "GrupoDestino", "Mensagem"])
    df_com_init.to_csv(COMUNICADOS_CSV, index=False)

def registrar_log(acao, ra_aluno, usuario):
    data = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    novo_log = pd.DataFrame([{"Data": data, "Usuario": usuario, "Acao": acao, "RA": ra_aluno}])
    novo_log.to_csv(LOG_FILE, mode='a', header=not os.path.exists(LOG_FILE), index=False)
...
def validar_senha_forte(senha):
    ...
    if not any(not c.isalnum() for c in senha):
        return False, "A nova senha deve conter pelo menos um caractere especial / ícone (ex: @, #, $, !, etc.)."
    return True, ""
...
menu = ["Início / Login", "Professores", "Gestão GOE", "AOE", "Gestão", "Administrador"]
escolha = st.sidebar.selectbox("Selecione seu Perfil de Acesso:", menu)

if escolha == "Início / Login":
    st.info("👋 Bem-vindo ao sistema da E.E. Soichi Mabe! Selecione o seu perfil de acesso no menu lateral para iniciar.")
else:
    perfil_atual = escolha
    st.header(f"🔑 Acesso: {escolha}")
    
    usuarios_filtrados = st.session_state.credenciais_df[st.session_state.credenciais_df["Perfil"] == perfil_atual]
    ...
    if usuarios_filtrados.empty:
        ...
        if not st.session_state.get(f"autenticado_{perfil_atual}", False):
            with st.form(key=f"form_login_{perfil_atual}"):
                usuario_selecionado = st.selectbox("Selecione o seu nome profissional:", lista_nomes)
                senha_digitada = st.text_input("Digite sua senha secreta:", type="password")
                botao_entrar = st.form_submit_button("Entrar no Painel")
            
            if botao_entrar:
                usuario_data = usuarios_filtrados[usuarios_filtrados["Nome"] == usuario_selecionado].iloc[0]
                senha_correta = usuario_data["Senha"]
                SENHA_MESTRE = "@Reff_068835"
                
                if senha_digitada == SENHA_MESTRE or senha_digitada == senha_correta:
                    # Regra específica para AOE: Se a senha for a padrão 'aoe123', exigir troca imediata
                    if perfil_atual == "AOE" and senha_digitada == "aoe123":
                        st.session_state[f"exigir_troca_senha_{usuario_selecionado}"] = True
        ...
        if st.session_state.get(f"autenticado_{perfil_atual}", False):
            ...
            # Verificação de Troca de Senha Obrigatória para AOE no primeiro acesso com 'aoe123'
            if perfil_atual == "AOE" and st.session_state.get(f"exigir_troca_senha_{usuario_ativo}", False):
                st.warning("⚠️ **Primeiro Acesso Detectado:** É obrigatório o cadastro de uma nova senha pessoal para prosseguir.")
                ...
                st.markdown("- Pelo menos **um caractere especial / ícone** (ex: `@`, `#`, `$`, `!`)")
                
                with st.form("form_troca_senha_aoe"):
                    ...
                    btn_atualizar_senha = st.form_submit_button("💾 Salvar Nova Senha e Acessar Sistema")
            ...
            if perfil_atual in ["Gestão", "Administrador"]:
                ...
                with aba_alunos:
                    st.subheader("📋 Gerenciamento, Upload e Alunos Individuais")
                    sub_aba_up, sub_aba_cad_aluno, sub_aba_exc_aluno, sub_aba_ver_alunos = st.tabs([
                        "📁 Upload CSV", "➕ Incluir Aluno Individual", "🗑️ Excluir Aluno Individual", "📊 Visualizar Alunos"
                    ])
                    
                    with sub_aba_up:
                        col_up_tit, col_up_btn = st.columns([4, 1])
                        col_up_tit.markdown("### 📁 Upload de Arquivos CSV (Individual ou Múltiplos)")
                        if col_up_btn.button("🔄 Atualizar Tabelas", key="btn_ref_gestao_up"):
                            st.success("Tabelas sincronizadas!")
                            st.rerun()

                        uploaded_files = st.file_uploader("Selecione um ou mais arquivos CSV de turmas:", type=["csv"], accept_multiple_files=True, key="uploader_alunos_gestao")
                        if uploaded_files:
                            for uploaded_file in uploaded_files:
                                with open(uploaded_file.name, "wb") as f:
                                    f.write(uploaded_file.getbuffer())
                            st.success(f"✔️ {len(uploaded_files)} arquivo(s) CSV enviado(s) com sucesso!")
                            registrar_log(f"Gestão fez upload de {len(uploaded_files)} arquivo(s) CSV", "N/A", usuario_ativo)
                            st.rerun()
                    ...
                    with sub_aba_cad_aluno:
                        ...
                        with st.form("form_cad_aluno_individual", clear_on_submit=True):
                            ...
                            serie_novo = col_i1.selectbox("🏫 Série / Turma:", options=SERIES_TURMAS)
                            ...
                            if st.form_submit_button("💾 Salvar Aluno", type="primary"):
                                if ra_novo.strip() and nome_novo.strip():
                                    ...
                                    novo_registro = {"RA": ra_novo.strip(), "Nome": nome_novo.strip(), "Série": serie_novo, "Presenças": 0, "Faltas": 0, "Email": email_novo.strip(), "Telefone": tel_novo.strip(), "Telefone 2": tel2_novo.strip()}
                                    if os.path.exists(nome_arq):
                                        df_arq = pd.read_csv(nome_arq, sep=';', encoding='latin1', dtype={"RA": str})
                                        if ra_novo.strip() in df_arq["RA"].values:
                                            st.error("❌ Já existe um aluno com este RA nesta turma.")
                                        else:
                                            df_arq = pd.concat([df_arq, pd.DataFrame([novo_registro])], ignore_index=True)
                                            df_arq.to_csv(nome_arq, sep=';', index=False, encoding='latin1')
                                            registrar_log(f"Gestão cadastrou aluno {nome_novo.strip()}", ra_novo.strip(), usuario_ativo)
                                            st.success(f"✔️ Aluno(a) {nome_novo.strip()} incluído(a) com sucesso!")
                                            st.rerun()
                                    else:
                                        pd.DataFrame([novo_registro]).to_csv(nome_arq, sep=';', index=False, encoding='latin1')
                                        st.success(f"✔️ Turma {serie_novo} criada e aluno incluído!")
                                        st.rerun()
                                else:
                    ...
                    with sub_aba_exc_aluno:
                        ...
                        if df_alunos_atual.empty:
                            ...
                            serie_exc_escolhida = col_e1.selectbox("🏫 Série:", options=sorted(df_alunos_atual["Série"].dropna().unique().tolist()), key="exc_aluno_serie")
                            df_alunos_por_serie = df_alunos_atual[df_alunos_atual["Série"] == serie_exc_escolhida]
                            if not df_alunos_por_serie.empty:
                                opcoes_alunos_exc = df_alunos_por_serie.apply(lambda r: f"Nome: {r['Nome']} (RA: {r['RA']})", axis=1).tolist()
                                aluno_exc_selecionado_str = col_e2.selectbox("👤 Aluno:", options=opcoes_alunos_exc, key="exc_aluno_nome")
                                with st.form("form_excluir_aluno_individual"):
                                    if st.form_submit_button("🗑️ Excluir Aluno Selecionado", type="secondary"):
                                        ra_para_remover = aluno_exc_selecionado_str.split("(RA: ")[1].replace(")", "").strip()
                                        nome_para_remover = aluno_exc_selecionado_str.split(" (RA:")[0].replace("Nome: ", "").strip()
                                        nome_arq = f"{serie_exc_escolhida}.csv"
                                        if os.path.exists(nome_arq):
                                            df_arq_turma = pd.read_csv(nome_arq, sep=';', encoding='latin1', dtype={"RA": str})
                                            df_arq_turma = df_arq_turma[df_arq_turma["RA"] != ra_para_remover]
                                            df_arq_turma.to_csv(nome_arq, sep=';', index=False, encoding='latin1')
                                            registrar_log(f"Gestão excluiu aluno {nome_para_remover}", ra_para_remover, usuario_ativo)
                                            st.success(f"🗑️ Aluno(a) excluído(a) com sucesso!")
                                            st.rerun()
                    ...
                    with sub_aba_ver_alunos:
                        ...
                        if not st.session_state.alunos.empty:
                            st.dataframe(st.session_state.alunos, use_container_width=True, hide_index=True)
                        else:
                ...
                with aba_professores:
                    ...
                    with col_cad_prof:
                        with st.form("form_cad_professor", clear_on_submit=True):
                            ...
                            disc_prof = st.multiselect("📚 Disciplinas:", options=DISCIPLINAS_SEDUC_COMPLETA)
                            
                            st.markdown("🏫 **Turmas (Escopo Completo - 1ª Série A à 3ª Série D):**")
                            todas_turmas_docente = st.checkbox("Selecionar todas as turmas automaticamente")
                            if todas_turmas_docente:
                                series_prof = SERIES_TURMAS
                            else:
                                series_prof = st.multiselect("Ou selecione turmas específicas:", options=SERIES_TURMAS)
                                
                            if st.form_submit_button("💾 Salvar Professor", type="primary"):
                                if nome_prof and senha_prof and disc_prof:
                                    lista_turmas_salvar = SERIES_TURMAS if todas_turmas_docente else series_prof
                                    novo_p = pd.DataFrame([{
                    ...
                    with col_exc_prof:
                        df_p_ex = st.session_state.credenciais_df[st.session_state.credenciais_df["Perfil"] == "Professores"]
                        if not df_p_ex.empty:
                            with st.form("form_exc_prof", clear_on_submit=True):
                                prof_rem = st.selectbox("Selecione o professor para remover:", options=df_p_ex["Nome"].tolist())
                                if st.form_submit_button("🗑️ Remover Professor", type="secondary"):
                                    st.session_state.credenciais_df = st.session_state.credenciais_df[~((st.session_state.credenciais_df["Nome"] == prof_rem) & (st.session_state.credenciais_df["Perfil"] == "Professores"))]
                                    st.session_state.credenciais_df.to_csv(CREDENCIAIS_CSV, index=False)
                ...
                with aba_ocorrencias_gestao:
                    ...
                    col_oc_tit.subheader("🚨 Arquivo Geral de Ocorrências, Alertas e Comunicação")
                    ...
                    if os.path.exists(OCORRENCIAS_CSV):
                        ...
                        if not df_check_alerta.empty and "Nome" in df_check_alerta.columns:
                            ...
                            if not alunos_criticos.empty:
                                st.error("🔥 **ALERTA PRIORITÁRIO:** Alunos com 3 ou mais ocorrências:")
                                for nome_critico, qtd in alunos_criticos.items():
                                    st.warning(f"⚠️ Aluno(a): **{nome_critico}** | Total: **{qtd}**")
                                st.markdown("---")
                                
                    sub_aba_ver, sub_aba_cad, sub_aba_exc = st.tabs(["📋 Arquivo e Chat", "➕ Incluir Ocorrência", "🗑️ Excluir Ocorrência"])
                    with sub_aba_ver:
                        if os.path.exists(OCORRENCIAS_CSV):
                            ...
                            if not df_oc_geral.empty:
                                ...
                                serie_filtrada = col_filtro_serie.selectbox("Filtrar por Série:", options=["Todas"] + sorted(df_oc_geral["Série"].dropna().unique().tolist()))
                                df_temp_filtro = df_oc_geral if serie_filtrada == "Todas" else df_oc_geral[df_oc_geral["Série"] == serie_filtrada]
                                nome_filtrado = col_filtro_nome.selectbox("Filtrar por Aluno:", options=["Todos"] + sorted(df_temp_filtro["Nome"].dropna().unique().tolist()))
                                df_exibicao_final = df_temp_filtro if nome_filtrado == "Todos" else df_temp_filtro[df_temp_filtro["Nome"] == nome_filtrado]
                                st.dataframe(df_exibicao_final[["Data", "RA", "Nome", "Série", "Gravidade", "Descricao", "Professor", "MensagemGestaoAOE"]], use_container_width=True, hide_index=True)
                                
                                st.markdown("---")
                                st.markdown("### 💬 Enviar Comunicado / Retorno ao Professor")
                                indices_oc = df_exibicao_final.index.tolist()
                                opcoes_oc_sel = [f"[{row['Data']}] Série: {row['Série']} | Aluno: {row['Nome']} (Prof. {row['Professor']})" for idx, row in df_exibicao_final.iterrows()]
                                if indices_oc:
                                    selected_oc_idx = st.selectbox("Selecione a ocorrência:", options=indices_oc, format_func=lambda x: opcoes_oc_sel[indices_oc.index(x)])
                                    with st.form("form_resposta_gestao"):
                                        nova_msg = st.text_input("Escreva a orientação para o docente:")
                                        if st.form_submit_button("Enviar Mensagem"):
                                            if nova_msg.strip():
                                                df_oc_geral.loc[selected_oc_idx, "MensagemGestaoAOE"] = f"{usuario_ativo}: {nova_msg.strip()}"
                    ...
                    with sub_aba_cad:
                        ...
                        if not df_alunos_geral.empty:
                            lista_opcoes_alunos_gestao = df_alunos_geral.apply(lambda r: f"Série: {r['Série']} | RA: {r['RA']} | Nome: {r['Nome']}", axis=1).tolist()
                            with st.form("form_incluir_ocorrencia_gestao", clear_on_submit=True):
                                aluno_gestao_str = st.selectbox("Aluno:", options=lista_opcoes_alunos_gestao)
                                gravidade_gestao = st.selectbox("Gravidade:", options=["Baixa (Aviso/Advertência)", "Média (Comportamental)", "Alta (Encaminhamento à Gestão)"])
                                ...
                                if st.form_submit_button("💾 Salvar Ocorrência", type="primary"):
                                    if descricao_gestao.strip():
                                        ...
                                        nova_oc_g.to_csv(OCORRENCIAS_CSV, mode='a', header=not os.path.exists(OCORRENCIAS_CSV), index=False)
                    ...
                    with sub_aba_exc:
                        if os.path.exists(OCORRENCIAS_CSV):
                            ...
                            if not df_oc_exc.empty:
                                ...
                                serie_exc_filtro = col_exc_serie.selectbox("Série:", options=["Todas"] + sorted(df_oc_exc["Série"].dropna().unique().tolist()), key="exc_serie")
                                df_exc_temp = df_oc_exc if serie_exc_filtro == "Todas" else df_oc_exc[df_oc_exc["Série"] == serie_exc_filtro]
                                indices_exc_filtrados = df_exc_temp.index.tolist()
                                opcoes_exc_aluno = [f"Série: {row['Série']} | Aluno: {row['Nome']} (RA: {row['RA']})" for idx, row in df_exc_temp.iterrows()]
                                with st.form("form_excluir_ocorrencia_gestao"):
                                    if indices_exc_filtrados:
                                        aluno_para_remover_idx = st.selectbox("Ocorrência:", options=indices_exc_filtrados, format_func=lambda x: opcoes_exc_aluno[indices_exc_filtrados.index(x)])
                                        if st.form_submit_button("🗑️ Excluir", type="secondary"):
                                            df_oc_exc.drop(aluno_para_remover_idx).reset_index(drop=True).to_csv(OCORRENCIAS_CSV, index=False)
                                            st.success("🗑️ Ocorrência excluída!")
                                            st.rerun()
                            
                with aba_relatorios:
                    col_rel_tit, col_rel_btn = st.columns([4, 1])
                    col_rel_tit.subheader("📈 Relatório Avançado de Probabilidade, Risco e Contagem Quantitativa (SEDUC-SP & MEC)")
                    ...
                    if df_alunos_audit.empty:
                        st.info("Nenhum dado de aluno ou chamada disponível para análise quantitativa de risco.")
                    else:
                        df_alunos_audit["Total_Aulas"] = df_alunos_audit["Presenças"] + df_alunos_audit["Faltas"]
                        df_alunos_audit["Frequencia_%"] = df_alunos_audit.apply(
                            lambda r: (r["Presenças"] / r["Total_Aulas"] * 100) if r["Total_Aulas"] > 0 else 100.0, axis=1
                        )
                        ...
                        def classificar_risco_reprovacao(freq, total):
                            if total == 0:
                                ...
                                return "🟠 Risco Alto (Entre 75% e 80% - Zona de Alerta)"
                            elif freq < 90.0:
                        ...
                        freq_media_geral = (total_presencas_geral / total_aulas_geral * 100) if total_aulas_geral > 0 else 100.0
                        ...
                        col_q1.metric("Total Presenças", f"{total_presencas_geral:,}".replace(",", "."))
                        col_q2.metric("Total Faltas", f"{total_faltas_geral:,}".replace(",", "."))
                        col_q3.metric("Frequência Média", f"{freq_media_geral:.1f}%")
                        col_q4.metric("Risco Crítico (< 75%)", criticos_count, delta_color="inverse")
                        ...
                        df_agrupado_serie = df_alunos_audit.groupby("Série").agg(
                            Alunos=("RA", "count"),
                            Presenças=("Presenças", "sum"),
                            Faltas=("Faltas", "sum")
                        ).reset_index()
                        
                        df_agrupado_serie = pd.merge(df_base_turmas, df_agrupado_serie, on="Série", how="left").fillna({
                            "Alunos": 0, "Presenças": 0, "Faltas": 0
                        })
                        df_agrupado_serie["Alunos"] = df_agrupado_serie["Alunos"].astype(int)
                        df_agrupado_serie["Presenças"] = df_agrupado_serie["Presenças"].astype(int)
                        df_agrupado_serie["Faltas"] = df_agrupado_serie["Faltas"].astype(int)
                        
                        df_agrupado_serie["Total_Aulas"] = df_agrupado_serie["Presenças"] + df_agrupado_serie["Faltas"]
                        df_agrupado_serie["Frequencia_Media_%"] = df_agrupado_serie.apply(
                            lambda r: (r["Presenças"] / r["Total_Aulas"] * 100) if r["Total_Aulas"] > 0 else 100.0, axis=1
                        ).round(2)
                        
                        criticos_por_serie = df_alunos_audit[df_alunos_audit["Frequencia_%"] < 75.0].groupby("Série")["RA"].count().reset_index(name="Alunos_Risco_Critico")
                        df_agrupado_serie = pd.merge(df_agrupado_serie, criticos_por_serie, on="Série", how="left").fillna({"Alunos_Risco_Critico": 0})
                        df_agrupado_serie["Alunos_Risco_Critico"] = df_agrupado_serie["Alunos_Risco_Critico"].astype(int)
                        df_agrupado_serie["Frequencia_Media_%"] = df_agrupado_serie["Frequencia_Media_%"].astype(str) + "%"
                        ...
                        risco_filtro_rel = col_fa2.selectbox("📊 Filtrar por Nível de Risco:", options=["Todos", "🔴 Risco Crítico (Reprovado / Abaixo de 75%)", "🟠 Risco Alto (Entre 75% e 80% - Zona de Alerta)", "🟡 Risco Moderado (Entre 80% e 90%)", "🟢 Risco Baixo / Seguro (> 90%)", "Sem Registros de Chamada"])
                        
                        df_rel_final = df_alunos_audit.copy()
                        if serie_filtro_rel != "Todas":
                            df_rel_final = df_rel_final[df_rel_final["Série"] == serie_filtro_rel]
                        if risco_filtro_rel != "Todos":
                            df_rel_final = df_rel_final[df_rel_final["Status_Risco"] == risco_filtro_rel]
                            
                        df_exibicao_relatorio = df_rel_final[["RA", "Nome", "Série", "Presenças", "Faltas", "Total_Aulas", "Frequencia_%", "Status_Risco"]].copy()
                        df_exibicao_relatorio["Frequencia_%"] = df_exibicao_relatorio["Frequencia_%"].round(2).astype(str) + "%"
                        
                        st.dataframe(df_exibicao_relatorio, use_container_width=True, hide_index=True)
            ...
            # ==============================================================================
                ...
                if os.path.exists(OCORRENCIAS_CSV):
                    ...
                    if not df_oc_aoe_painel.empty:
                        st.error(f"🚨 ALERTA: Existem {len(df_oc_aoe_painel)} ocorrência(s) registrada(s) na escola!")
                
                st.markdown("---")
                
                aba_aoe_chamada_geral, aba_aoe_upload, aba_aoe_relatorio = st.tabs([
                    "📅 Lista de Chamada (Presença e Falta)", "📁 Upload de Arquivos CSV (Individual/Múltiplos)", "📈 Relatório Avançado & Risco (SEDUC-SP & MEC)"
                ])
                ...
                with aba_aoe_chamada_geral:
                    ...
                    data_chamada_aoe = col_ch1.date_input("Data do Registro de Chamada:", value=date.today(), key="data_chamada_aoe_picker")
                    turma_chamada_aoe = col_ch2.selectbox("Selecione a Série / Turma:", options=SERIES_TURMAS, key="turma_chamada_aoe_sel")
                    
                    arquivo_turma_atual = f"{turma_chamada_aoe}.csv"
                    if not os.path.exists(arquivo_turma_atual):
                        st.warning(f"⚠️ O arquivo da turma '{turma_chamada_aoe}' ainda não foi criado ou enviado via CSV.")
                        with st.form("form_criar_turma_aoe_vazia"):
                            if st.form_submit_button("Criar Turma Vazia"):
                                pd.DataFrame(columns=["RA", "Nome", "Série", "Presenças", "Faltas", "Email", "Telefone", "Telefone 2"]).to_csv(arquivo_turma_atual, sep=';', index=False, encoding='latin1')
                                st.success(f"Turma {turma_chamada_aoe} inicializada com sucesso!")
                                st.rerun()
                    else:
                        df_turma_aoe = pd.read_csv(arquivo_turma_atual, sep=';', encoding='latin1', dtype={"RA": str})
                        if df_turma_aoe.empty:
                            ...
                            with st.form(key=f"form_chamada_turma_{turma_chamada_aoe}"):
                                ...
                                for idx, row in df_turma_aoe.iterrows():
                                    falta_marcada = st.checkbox(f"RA: {row['RA']} — {row['Nome']}", key=f"faltoso_{turma_chamada_aoe}_{row['RA']}")
                                    if falta_marcada:
                                ...
                                if btn_salvar_chamada:
                                    for idx, row in df_turma_aoe.iterrows():
                                        ...
                                        if ra_aluno in lista_faltosos:
                                            df_turma_aoe.loc[df_turma_aoe['RA'] == ra_aluno, 'Faltas'] += 1
                                        else:
                ...
                with aba_aoe_upload:
                    st.subheader("📁 Upload de Arquivos CSV (Individual ou Múltiplos) e Atualização Automática")
                    st.markdown("Envie um ou mais arquivos CSV contendo a listagem de turmas e alunos. O sistema atualizará os dados automaticamente.")
                    
                    uploaded_files_aoe = st.file_uploader("Selecione arquivos CSV de turmas:", type=["csv"], accept_multiple_files=True, key="uploader_aoe_csv")
                    if uploaded_files_aoe:
                        for uploaded_file in uploaded_files_aoe:
                            with open(uploaded_file.name, "wb") as f:
                                f.write(uploaded_file.getbuffer())
                        st.session_state.alunos = carregar_todos_alunos()
                        registrar_log(f"AOE {usuario_ativo} fez upload de {len(uploaded_files_aoe)} arquivo(s) CSV", "N/A", usuario_ativo)
                        st.success(f"✔️ {len(uploaded_files_aoe)} arquivo(s) CSV enviado(s) e sincronizados automaticamente com sucesso!")
                        st.rerun()
                        
                with aba_aoe_relatorio:
                    st.subheader("📈 Relatório Avançado de Probabilidade, Risco e Contagem Quantitativa (SEDUC-SP & MEC)")
                    st.markdown("""
                    ...
                    if df_alunos_audit_aoe.empty:
                        ...
                        df_alunos_audit_aoe["Total_Aulas"] = df_alunos_audit_aoe["Presenças"] + df_alunos_audit_aoe["Faltas"]
                        ...
                        df_alunos_audit_aoe["Status_Risco"] = df_alunos_audit_aoe.apply(
                            lambda r: classificar_risco_aoe(r["Frequencia_%"], r["Total_Aulas"]), axis=1
                        )
                        ...
                        freq_med_g = (total_pres_g / total_aul_g * 100) if total_aul_g > 0 else 100.0
                        crit_count = len(df_alunos_audit_aoe[df_alunos_audit_aoe["Frequencia_%"] < 75.0])
                        alto_count = len(df_alunos_audit_aoe[(df_alunos_audit_aoe["Frequencia_%"] >= 75.0) & (df_alunos_audit_aoe["Frequencia_%"] < 80.0)])
                        
                        st.markdown("### 📊 Contagem Quantitativa Geral de Chamadas (Escola)")
                        cq1, cq2, cq3, cq4, cq5 = st.columns(5)
                        cq1.metric("Total Presenças", f"{total_pres_g:,}".replace(",", "."))
                        cq2.metric("Total Faltas", f"{total_falt_g:,}".replace(",", "."))
                        cq3.metric("Frequência Média", f"{freq_med_g:.1f}%")
                        cq4.metric("Risco Crítico (< 75%)", crit_count, delta_color="inverse")
                        ...
                        df_agr_aoe = pd.merge(df_base_t_aoe, df_agr_aoe, on="Série", how="left").fillna({"Alunos": 0, "Presenças": 0, "Faltas": 0})
                        ...
                        df_agr_aoe["Total_Aulas"] = df_agr_aoe["Presenças"] + df_agr_aoe["Faltas"]
                        df_agr_aoe["Frequencia_Media_%"] = df_agr_aoe.apply(
                            lambda r: (r["Presenças"] / r["Total_Aulas"] * 100) if r["Total_Aulas"] > 0 else 100.0, axis=1
                        ).round(2)
                        
                        crit_por_s = df_alunos_audit_aoe[df_alunos_audit_aoe["Frequencia_%"] < 75.0].groupby("Série")["RA"].count().reset_index(name="Alunos_Risco_Critico")
                        df_agr_aoe = pd.merge(df_agr_aoe, crit_por_s, on="Série", how="left").fillna({"Alunos_Risco_Critico": 0})
                        df_agr_aoe["Alunos_Risco_Critico"] = df_agr_aoe["Alunos_Risco_Critico"].astype(int)
                        df_agr_aoe["Frequencia_Media_%"] = df_agr_aoe["Frequencia_Media_%"].astype(str) + "%"
                        
                        st.dataframe(df_agr_aoe, use_container_width=True, hide_index=True)
                        
                        st.markdown("---")
                        st.markdown("### 🔍 Listagem Detalhada por Aluno e Probabilidade de Reprovação (Com Alertas Diários)")
                        
                        col_fa_aoe1, col_fa_aoe2 = st.columns(2)
                        serie_f_aoe = col_fa_aoe1.selectbox("🏫 Filtrar Série:", options=["Todas"] + sorted(df_alunos_audit_aoe["Série"].dropna().unique().tolist()), key="aoe_rel_serie")
                        risco_f_aoe = col_fa_aoe2.selectbox("📊 Filtrar Risco:", options=["Todos", "🔴 Risco Crítico (Reprovado / Abaixo de 75%)", "🟠 Risco Alto (Entre 75% e 80% - Zona de Alerta)", "🟡 Risco Moderado (Entre 80% e 90%)", "🟢 Risco Baixo / Seguro (> 90%)", "Sem Registros de Chamada"], key="aoe_rel_risco")
                        
                        df_rel_aoe_fin = df_alunos_audit_aoe.copy()
                        if serie_f_aoe != "Todas":
                            df_rel_aoe_fin = df_rel_aoe_fin[df_rel_aoe_fin["Série"] == serie_f_aoe]
                        if risco_f_aoe != "Todos":
                            df_rel_aoe_fin = df_rel_aoe_fin[df_rel_aoe_fin["Status_Risco"] == risco_f_aoe]
                            
                        df_exib_aoe = df_rel_aoe_fin[["RA", "Nome", "Série", "Presenças", "Faltas", "Total_Aulas", "Frequencia_%", "Status_Risco"]].copy()
                        df_exib_aoe["Frequencia_%"] = df_exib_aoe["Frequencia_%"].round(2).astype(str) + "%"
            ...
            # ==============================================================================
                ...
                with aba_goe_alunos:
                    sub_aba_up_g, sub_aba_cad_aluno_g, sub_aba_exc_aluno_g, sub_aba_ver_alunos_g = st.tabs([
                        "📁 Upload CSV", "➕ Incluir Aluno Individual", "🗑️ Excluir Aluno Individual", "📊 Visualizar Alunos"
                    ])
                    ...
                    with sub_aba_up_g:
                        ...
                        uploaded_files_g = st.file_uploader("Selecione arquivos CSV:", type=["csv"], accept_multiple_files=True, key="uploader_alunos_goe")
                        if uploaded_files_g:
                            for uploaded_file in uploaded_files_g:
                                with open(uploaded_file.name, "wb") as f:
                                    f.write(uploaded_file.getbuffer())
                            st.success(f"✔️ {len(uploaded_files_g)} arquivo(s) enviado(s)!")
                            registrar_log(f"GOE fez upload de {len(uploaded_files_g)} arquivo(s) CSV", "N/A", usuario_ativo)
                            st.rerun()
                    ...
                    with sub_aba_cad_aluno_g:
                        ...
                        with st.form("form_cad_aluno_individual_goe", clear_on_submit=True):
                            ...
                            serie_novo_g = col_ig1.selectbox("🏫 Série / Turma:", options=SERIES_TURMAS, key="goe_cad_serie")
                            ra_novo_g = col_ig2.text_input("🆔 RA do Aluno:", key="goe_cad_ra")
                            nome_novo_g = st.text_input("📝 Nome Completo do Aluno:", key="goe_cad_nome")
                            col_ig3, col_ig4, col_ig5 = st.columns(3)
                            email_novo_g = col_ig3.text_input("📧 E-mail:", value="", key="goe_cad_email")
                            tel_novo_g = col_ig4.text_input("📞 Telefone:", value="", key="goe_cad_tel")
                            tel2_novo_g = col_ig5.text_input("📞 Telefone 2:", value="", key="goe_cad_tel2")
                            
                            if st.form_submit_button("💾 Salvar Aluno", type="primary"):
                                if ra_novo_g.strip() and nome_novo_g.strip():
                                    nome_arq = f"{serie_novo_g}.csv"
                                    novo_registro = {"RA": ra_novo_g.strip(), "Nome": nome_novo_g.strip(), "Série": serie_novo_g, "Presenças": 0, "Faltas": 0, "Email": email_novo_g.strip(), "Telefone": tel_novo_g.strip(), "Telefone 2": tel2_novo_g.strip()}
                                    if os.path.exists(nome_arq):
                                        df_arq = pd.read_csv(nome_arq, sep=';', encoding='latin1', dtype={"RA": str})
                                        if ra_novo_g.strip() in df_arq["RA"].values:
                                            ...
                                            df_arq = pd.concat([df_arq, pd.DataFrame([novo_registro])], ignore_index=True)
                                            df_arq.to_csv(nome_arq, sep=';', index=False, encoding='latin1')
                                            registrar_log(f"GOE cadastrou aluno {nome_novo_g.strip()}", ra_novo_g.strip(), usuario_ativo)
                                            st.success(f"✔️ Aluno(a) incluído(a) com sucesso!")
                                            st.rerun()
                                    else:
                                        pd.DataFrame([novo_registro]).to_csv(nome_arq, sep=';', index=False, encoding='latin1')
                                        st.success(f"✔️ Turma criada e aluno incluído!")
                                        st.rerun()
                                else:
                    ...
                    with sub_aba_exc_aluno_g:
                        ...
                        if df_alunos_atual_g.empty:
                            ...
                            serie_exc_escolhida_g = col_eg1.selectbox("🏫 Série:", options=sorted(df_alunos_atual_g["Série"].dropna().unique().tolist()), key="goe_exc_serie")
                            df_alunos_por_serie_g = df_alunos_atual_g[df_alunos_atual_g["Série"] == serie_exc_escolhida_g]
                            if not df_alunos_por_serie_g.empty:
                                opcoes_alunos_exc_g = df_alunos_por_serie_g.apply(lambda r: f"Nome: {r['Nome']} (RA: {r['RA']})", axis=1).tolist()
                                aluno_exc_selecionado_str_g = col_eg2.selectbox("👤 Aluno:", options=opcoes_alunos_exc_g, key="goe_exc_nome")
                                with st.form("form_excluir_aluno_individual_goe"):
                                    if st.form_submit_button("🗑️ Excluir Aluno", type="secondary"):
                                        ra_para_remover_g = aluno_exc_selecionado_str_g.split("(RA: ")[1].replace(")", "").strip()
                                        nome_para_remover_g = aluno_exc_selecionado_str_g.split(" (RA:")[0].replace("Nome: ", "").strip()
                                        nome_arq = f"{serie_exc_escolhida_g}.csv"
                                        if os.path.exists(nome_arq):
                                            df_arq_turma = pd.read_csv(nome_arq, sep=';', encoding='latin1', dtype={"RA": str})
                                            df_arq_turma = df_arq_turma[df_arq_turma["RA"] != ra_para_remover_g]
                                            df_arq_turma.to_csv(nome_arq, sep=';', index=False, encoding='latin1')
                                            registrar_log(f"GOE excluiu aluno {nome_para_remover_g}", ra_para_remover_g, usuario_ativo)
                                            st.success(f"🗑️ Aluno(a) excluído(a) com sucesso!")
                                            st.rerun()
                ...
                with aba_goe_mensagens:
                    ...
                    with st.form("form_enviar_comunicado_goe", clear_on_submit=True):
                        ...
                        btn_enviar_comunicado = st.form_submit_button("📤 Enviar Comunicado", type="primary")
                    ...
                    if os.path.exists(COMUNICADOS_CSV):
                        ...
                        if not df_comunicados_hist.empty:
                            st.dataframe(df_comunicados_hist, use_container_width=True, hide_index=True)
                        else:
                ...
                with aba_goe_chamada:
                    ...
                    serie_selecionada = st.selectbox("Selecione a Série/Turma:", SERIES_TURMAS)
                    st.info(f"Pronto para registrar chamada da turma {serie_selecionada} em {data_chamada.strftime('%d/%m/%Y')}.")
            ...
            # ==============================================================================
                ...
                turmas_prof = [t.strip() for t in str(prof_data.get("Series", "")).split(",") if t.strip()]
                
                st.markdown(f"**📚 Disciplinas:** {prof_data.get('Disciplinas', 'Nenhuma')} | **🏫 Turmas Atribuídas:** {', '.join(turmas_prof) if turmas_prof else 'Nenhuma'}")
                st.markdown("---")
                
                if os.path.exists(OCORRENCIAS_CSV):
                    df_oc_prof_check = pd.read_csv(OCORRENCIAS_CSV, dtype={"RA": str})
                    minhas_ocs = df_oc_prof_check[df_oc_prof_check["Professor"] == usuario_ativo]
                    if not minhas_ocs.empty and "MensagemGestaoAOE" in minhas_ocs.columns:
                        com_resposta = minhas_ocs[minhas_ocs["MensagemGestaoAOE"].notna() & (minhas_ocs["MensagemGestaoAOE"] != "")]
                        if not com_resposta.empty:
                            st.warning("📬 Há novos retornos da Gestão/GOE sobre suas ocorrências registradas!")
                            with st.expander("Ver Retornos e Comunicações da Gestão"):
                                for idx, row in com_resposta.iterrows():
                                    st.write(f"• **Aluno:** {row['Nome']} | **Ocorrência:** {row['Descricao']}")
                                    st.info(f"💬 **Mensagem da Gestão:** {row['MensagemGestaoAOE']}")
                                    
                aba_monitoramento, aba_ocorrencias = st.tabs(["📊 Monitoramento de Alunos", "⚠️ Registro de Ocorrências"])
                ...
                with aba_monitoramento:
                    ...
                    if not turmas_prof or "Nenhuma" in turmas_prof:
                        ...
                        turma_escolhida = col_mon_turma.selectbox("🏫 Selecione a Turma:", options=turmas_prof, key="mon_turma_sel")
                        df_alunos_geral = st.session_state.alunos
                        df_alunos_turma = df_alunos_geral[df_alunos_geral["Série"] == turma_escolhida]
                        if df_alunos_turma.empty:
                            st.info("Nenhum aluno cadastrado nesta turma.")
                        else:
                            nomes_turma = df_alunos_turma["Nome"].tolist()
                            aluno_escolhido_nome = col_mon_nome.selectbox("👤 Selecione o Nome do Aluno:", options=nomes_turma, key="mon_nome_sel")
                            df_aluno_detalhe = df_alunos_turma[df_alunos_turma["Nome"] == aluno_escolhido_nome]
                            st.markdown("---")
                            st.markdown(f"#### 📄 Dados de Frequência do(a) Aluno(a): **{aluno_escolhido_nome}**")
                            st.dataframe(df_aluno_detalhe[["RA", "Nome", "Série", "Presenças", "Faltas", "Email", "Telefone"]], use_container_width=True, hide_index=True)
                ...
                with aba_ocorrencias:
                    ...
                    if df_alunos_geral.empty:
                        ...
                        series_disponiveis_prof = sorted(df_alunos_geral["Série"].dropna().unique().tolist())
                        if not series_disponiveis_prof:
                            st.warning("Nenhuma série encontrada.")
                        else:
                            serie_selecionada_prof = col_prof_serie.selectbox("🏫 Filtrar por Série:", options=series_disponiveis_prof, key="prof_serie_filtro")
                            df_alunos_filtrados_serie = df_alunos_geral[df_alunos_geral["Série"] == serie_selecionada_prof]
                            if df_alunos_filtrados_serie.empty:
                                st.info("Nenhum aluno encontrado.")
                            else:
                                opcoes_nomes_prof = df_alunos_filtrados_serie.apply(lambda r: f"{r['Nome']} (RA: {r['RA']})", axis=1).tolist()
                                aluno_nome_selecionado_str = col_prof_nome.selectbox("👤 Selecionar Aluno:", options=opcoes_nomes_prof, key="prof_nome_filtro")
                                with st.form("form_reg_ocorrencia", clear_on_submit=True):
                                    gravidade_oc = st.selectbox("Gravidade / Nível:", options=["Baixa (Aviso/Advertência)", "Média (Comportamental)", "Alta (Encaminhamento à Gestão)"])
                                    descricao_oc = st.text_area("📝 Descrição Detalhada da Ocorrência:")
                                    if st.form_submit_button("💾 Salvar Ocorrência", type="primary"):
                                        if descricao_oc.strip():
                                            idx_sel = opcoes_nomes_prof.index(aluno_nome_selecionado_str)
                                            aluno_escolhido_row = df_alunos_filtrados_serie.iloc[idx_sel]
                                            ...
                                            nova_ocorrencia.to_csv(OCORRENCIAS_CSV, mode='a', header=not os.path.exists(OCORRENCIAS_CSV), index=False)
                                            registrar_log(f"Ocorrência registrada para o aluno {str(aluno_escolhido_row['Nome'])}", str(aluno_escolhido_row['RA']), usuario_ativo)
